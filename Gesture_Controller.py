...
def main():
    ...
    result=""
    if st.button("Gesture Controller"):
        result = GestureController()
        result.start()
    if st.button("Proton"):
        # st.text("Lets Learn")
        ...
        t1.start()
    if st.button("ABOUT"):
        st.text("Gesture Controlled Virtual Mouse simplifies human computer interaction by utilizing \nHand Gestures and Voice Commands. Almost no direct contact is required with \nthe computer. All I/O operations can be controlled remotely via \nstatic and dynamic hand gestures.")
